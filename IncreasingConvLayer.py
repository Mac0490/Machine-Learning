...
class NetSeq(nn.Module):
    def __init__(self):
        ...
        self.cnn_layers = nn.Sequential(
            nn.Conv2d(1, 32, 3), nn.ReLU(), nn.MaxPool2d(2),  # Original
            nn.Conv2d(32, 64, 3), nn.ReLU(), nn.MaxPool2d(2),  # Original
            nn.Conv2d(64, 128, 3), nn.ReLU(),  # New layer
            nn.Dropout(0.2)
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(128*3*3, 10),  # Adjusted for new dimensions
            nn.LogSoftmax(dim=1)
        )
...
if __name__ == "__main__":
    ...
    # Increasing Convolutional Layers: Slight accuracy boost, training time was around 50 percent slovewer, Risk of overfitting, only worth for complex datasets, minimal gain for MNIST