# Imports
# Replace the final Softmax layer with Sigmoid
import argparse
...
# Replace the final Softmax layer with Sigmoid: How do the classification results change? accuracy crashed, there were a lot of more errors in mislasification plot

# What does this mean for the interpretation of the network outputs?  Sigmoid treats classes independently, outputs are not coparable probabilities , nwetwork fails to learn digit distinctions
#Sigmoid is for multi-label tasks, not MNIST's single-label classification. Always use Softmax for mutually exclusive classes.