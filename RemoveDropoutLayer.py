...
#Result- Trainig curves: I observed faster convergence, slightly higher test accuracy, and train gap is smaller than expected

#Result- What does a dropout layer do and when does it influence the functioning of a neural network: 
# Prevents overfitting, does not harm convergence
# A dropout layer randomly deactivates neurons during training to prevent overfitting by forcing the network to learn redundant features. It significantly influences neural networks when model is deep/complex (many layers), training data is limited or noisy, overfitting is observed (large gap between train/test accuracy) 
# 
# In your MNIST case, dropout had minimal impact because the dataset is simple and the network is shallow. It iss most useful in larger, more complex models.