...
# Training Metrics and Epoch Behavior = >
# Training Loss: Decreases steadily from ~0.13 to ~0.03 over 4 epochs, 
# Accuracy: Both train and test accuracy improve (train: 98%→99.4%, test: 97.8%→99.0%)
# Epoch Behavior: The model converges quickly with strong initial improvements, there is small gap between train/test accuracy suggests good generalization
# Many Epochs Risk: Overfitting, diminishing returns in performance gains

#Misclassification Analysis => Most errors occur with visually similar digits: 3s misclassified as 5s/8s, 5s as 3s/6s, 7s as 1s/9s
#Problematic cases usually have, unusual handwritting styles, heavy slant or rotation or ambiguous strokes

#Softmax activation Purpose =>
# Enables classification across 10 digit classes, normalization, outputs can be directly read as class probabilities
